...
class _ClientListState extends State<ClientList> {
  ...
  Widget _stream(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
        stream: Firestore.instance.collection('orders').document('SQujodVWeKgohCENPueX').snapshots(),
        builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
          if (!snapshot.hasData){
            return Container(
                padding: EdgeInsets.only(top:15.0),
                alignment: Alignment.topCenter,
                child: Text('No clients found', style: TextStyle(fontSize: 15),)
            );
          }
  ...
  undoDelete(doc, key) async{
    doc['volunteer_id'] = 0;
    await Firestore.instance.collection("orders").document('SQujodVWeKgohCENPueX').updateData({key: doc});
    setState(() {});
  }
